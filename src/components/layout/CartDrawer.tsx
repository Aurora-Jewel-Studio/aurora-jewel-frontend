            ...
            <div className="flex-1 overflow-y-auto p-6">
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center opacity-70">
                  <ShoppingBag
                    size={48}
                    className="mb-4 text-[var(--color-brand-accent)]"
                  />
                  <p className="font-serif text-xl mb-2">Your cart is empty</p>
                  <p className="text-sm">
                    Discover our exquisite collections to find your next
                    treasure.
                  </p>
                  <Button
                    onClick={() => setIsCartOpen(false)}
                  ...
                  {items.map((item) => (
                    <li key={item.variantId} className="flex gap-4">
                      <div className="w-24 h-24 bg-[var(--bg-secondary)] rounded-md overflow-hidden relative flex-shrink-0">
                        <img
                          src={item.thumbnail || "/images/hero-jewelry.png"}
                          alt={item.title}
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        <div>
                          <div className="flex justify-between items-start">
                            <h3 className="font-serif text-lg leading-tight pr-4">
                              {item.title}
                            </h3>
                            <button
                        ...
                        <div className="flex justify-between items-center mt-2">
                          <div className="flex items-center border border-[var(--border-color)] rounded-md">
                            ...
                            <span className="px-2 text-sm min-w-8 text-center">
                              {item.quantity}
                            </span>
                            <button
                              onClick={() =>
